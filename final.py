...
url='http://172.16.246.251:8080/shot.jpg'
time.sleep(2.0)

known_face_name =["manirajan","sathis","keerthi","elakkiya","Gowtham","manick","Rajagopal","savitha","SENTNIL K","Raghul","Ragavi","koki","aishwarya","vigenesh"]
            ...
            with open('student.csv', 'r') as csvFile:
                ...
                if name in listString:
                    if name in students:
                        ...
                        Dept = singleList[Index + 2]
                        mail = singleList[Index + 3]
                        #print(Roll_Number)
                        d = datetime.now()
                        current_time = d.strftime("%H-%M-%S")
                        current_date = d.strftime("%Y-%m-%d")
                    
                        Inwriter.writerow([str(name), str(Roll_Number), str(Dept), str(mail), str(current_time), str(current_date)])
                        csvFile.close()
